...
void FactoryVisitor::AddBackground(BBModel* model) 
{
	...
	Texture::LoadBitmap("content/backgrounds/best/stars.bmp", backgroundModel->cacheTexture);
...
{
	model->addPlayer(new BBPlayerModel(model));
	model->addPlayer(new BBPlayerModel(model));
}
...
*/
	for(int i = 0; i < 1; i++)
		for(int j = 0; j < numShips; j++)
		{
			...
			BBShipModel* ship = new BBShipModel(i*80, j*40+200, 0, 1, models[j], playerModel, objs[j]);
